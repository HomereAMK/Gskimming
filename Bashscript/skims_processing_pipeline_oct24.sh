...
cores="${cores:-8}"
interleaven_counter="${interleaven_counter:-0}"

if [ -z "$input" ]; then
    echo "Error: The -x option (input directory) must be provided."
    echo "$usage"
    exit 1
fi
...
for filepath in "${input}"/*; do
    ...
    if [[ "$extension" =~ ^(fastq|fq|fna|fa)$ ]]; then
        filename_no_ext="${filename%.*}"  # Remove extension
        # Extract base genome name and mate number
        if [[ "$filename_no_ext" =~ ^(.*)_([12])$ ]]; then
            base_genome="${BASH_REMATCH[1]}"
            mate="${BASH_REMATCH[2]}"
            if [ "$mate" = "1" ]; then
                R1_files["$base_genome"]="$filepath"
            elif [ "$mate" = "2" ]; then
                R2_files["$base_genome"]="$filepath"
            fi
        else
        ...
        echo "File $filename has unsupported extension. Skipping."
        continue
    fi
done
...
mkdir -p "${out_dir}/${direc_name}/bbmap"
mkdir -p "${out_dir}/${direc_name}/assemblies"
mkdir -p "${out_dir}/${direc_name}/consult"
mkdir -p "${out_dir}/${direc_name}/kraken"
mkdir -p "${out_dir}/${direc_name}/respect"
...
for genome in "${!R1_files[@]}"; do
    assembly_counter=0
    input_1="${R1_files[$genome]}"
    input_2="${R2_files[$genome]}"

    if [ -z "$input_2" ]; then
        echo "Paired file for $input_1 not found. Skipping."
        continue
    fi
    ...
    if [ "$extension_1" = "gz" ]; then
        ...
        if [ "$extension_2" = "fna" ] || [ "$extension_2" = "fa" ]; then
            assembly_counter=1
            zcat "$input/$input_1" > "${out_dir}/${direc_name}/assemblies/${genome}.fna"
        fi
    ...
    if [ -f "${out_dir}/${direc_name}/bbmap/${genome}_merged.fq.gz" ] && [ "0" -eq "$assembly_counter" ]; then
        test_var=0
        test_var=$(grep "$genome" "$SCRIPT_DIR/log_file.txt" | grep -c -e "A$" -e "AD$" -e "ADE$" -e "ADEB$" -e "ADEBC$") || true
        if [ "1" -eq "$test_var" ]; then
            counter=1
        else
            rm "${out_dir}/${direc_name}/bbmap/${genome}_merged.fq.gz"
        fi
    fi
    ...
    if [ "0" -eq "$counter" ] && [ "0" -eq "$assembly_counter" ]; then
        ...
        if [ "$extension" = "gz" ]; then
            # Call the updated bbmap_pipeline_oct24.sh
            "${SCRIPT_DIR}/bbmap_pipeline_oct24.sh" \
                "${input_1}" \
                "${input_2}" \
                "${out_dir}/${direc_name}/bbmap/${genome}_merged.fq.gz"

            echo "BBMap cleanup pipeline ends"
        else
            echo "Processing non-gzipped input files"
    ...
    if [ "0" -eq "$counter" ] && [ "0" -eq "$assembly_counter" ]; then
        ...
        ./consult_search --unclassified-out \
            -i all_nbrhood_kmers_k32_p3l2clmn7_K15-map2-171_ToL/ \
            -q "${out_dir}/${direc_name}/bbmap/${genome}_merged.fq.gz" \
            -o "${out_dir}/${direc_name}/consult/"
    ...
    if [ "0" -eq "$counter" ]; then
        if [ "0" -eq "$assembly_counter" ]; then
            "${SCRIPT_DIR}/skmer_pipeline.sh" "$f" \
                "${out_dir}/${direc_name}/kraken" \
                "unclassified-kra_${genome}.fq" "$lib_dir" "$cores"
        else
            if [ "$extension_1" = "gz" ]; then
                "${SCRIPT_DIR}/skmer_pipeline.sh" "$f" \
                    "${out_dir}/${direc_name}/assemblies" \
                    "${genome}.fna" "$lib_dir" "$cores"
            else
                "${SCRIPT_DIR}/skmer_pipeline.sh" "$f" \
                    "${input}" "${input_1}" "$lib_dir" "$cores"
            fi
        fi
    ...
    if [ "0" -eq "$counter" ]; then
        if [ "0" -eq "$assembly_counter" ]; then
            "${SCRIPT_DIR}/respect_pipeline.sh" "$cov_val" "$genome" "$lib_dir" \
                "${out_dir}/${direc_name}" "$iterations" "$threads" "$assembly_counter" \
                "$extension_1" "${out_dir}/${direc_name}"
        else
            if [ "$extension_1" = "gz" ]; then
                "${SCRIPT_DIR}/respect_pipeline.sh" "$cov_val" "$genome" "$lib_dir" \
                    "${out_dir}/${direc_name}" "$iterations" "$threads" "$assembly_counter" \
                ...
                "${SCRIPT_DIR}/respect_pipeline.sh" "$cov_val" "$genome" "$lib_dir" \
                    "${input}" "$iterations" "$threads" "$assembly_counter" \
                    "$extension_1" "${out_dir}/${direc_name}"
            fi
        fi
